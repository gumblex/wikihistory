...
ignoredTags = [
        'big', 'blockquote', 'center', 'cite', 'div', 
        'font', 'h1', 'h2', 'h3', 'h4', 'hiero', 'kbd', 'nowiki',
        'p', 'plaintext', 'small', 'span',
        'tt', 'var',
        #'b', 'i', 'u', 'em', 'strong', 'sub', 'sup', 's', 'strike',
]
...
templatefirstfield = re.compile(r'\{\{[^|=]+?\|([^=]+?)\|.+?\}\}')
templateonlyfield = re.compile(r'\{\{[^|=]+?\|([^=]+?)\}\}')
templateerr = re.compile(r'{{(citation needed|fact|cn|Attribution needed|By whom|Which|Who|link-[a-z-]+)\|([^=]+?)(\|.+?)?}}')
templatelang = re.compile(r'\{\{lang\|.+?\|(.+?)\}\}')
...
def clean(text):
    ...
    text = templateerr.sub(r'\1', text)
    text = templatelang.sub(r'\1', text)
    #text = templateonlyfield.sub(r'\1', text)
    ...
    text = italic_quote.sub(r'&quot;<i>\1</i>&quot;', text)
...
def compact(text):
    ...
    for line in text.split('\n'):
        ...
        # handle lists
            ...
            if keepSections:
                page.append('<li>%s</li>' % line) #line[1:]
            else:
                continue
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            if (colon < 0 or title[:colon] in acceptedNamespaces) and \
                    not redirect:
                if 1<len(title)< 8:
                    if re.match('^(|第|前)\d+(千年|年|世紀|世纪|年代)$', title):  ########################
                        print(id, title, file=sys.stderr)    #.encode('utf-8')
                        # sys.stdout.flush()
...
def main():
    ...
    output_splitter = DummyOutputSplitter(compress, file_size, output_dir)
